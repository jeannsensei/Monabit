...
export function createApp() {
  ...
  app.use((req, _res, next) => {
    logger.info({ method: req.method, path: req.path, requestId: req.requestId }, 'Request received');
    next();
  });
  ...
  app.get('/api/health', async (_req, res) => {
    ...
    res.json({
      ...
      services: {
        ...
        cache: { status: 'ok', keys: cacheStats.keys, hits: cacheStats.hits, misses: cacheStats.misses },
      },
      version: '1.0.0',
    });
  });
